    		...
    		<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
   			...
   			<link rel="stylesheet" type="text/css" href="../Lib_bootstrap/dist/sweetalert.css">
		   			...
		   			$out_off=$con->query("select * from offrande 
		   				inner join evenement on offrande.evenement=evenement.eid where signataire_un is NOT NULL and signataire_deux='$id' and signataire_trois is NULL and oid='$oid';") or die(print_r($con->errorInfo()));
		   			$fetch_off=$out_off->fetch();
		   			$count_row=$out_off->rowCount();
		   			if($count_row<=0){
		   				header('location:alert_used_b.php');
    					exit();
		   			}
		   			?>
...
var dataString = "oid="+oid +"&id="+id +"&date="+ date + "&culte="+ culte +"&rec="+ rec + "&culte_alt=" + culte_alt;
...
$.ajax({
type: "POST",
url: "modifier_offrande_rej_eng.php",
data:dataString,
cache:true,
