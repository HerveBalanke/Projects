<?php
					...
					include_once("session_deux.php");
		   			...
		   			$insert_membre=$con->prepare("update membre set nom=?, prenom=?, genre=?, dob=?, nbre_denfant=?, pays=?, profession=?, filiere=?, 
		   										ecole=?, sit_mat=?, codetel=?, tel=?, codewhat=?, whatsapp=?, email=?, facebook=?, address=?, activite_ac=?, info_spi=?, per_urgence=?, date_adhesion=? 
		   			...
		   			$insert_membre->execute(array( $nom, $prenom, $genre, $date, $ne, $pays, $prof, $fil, $ecole, $sim, $codetel, $tel, 
		   											$codewhat, $whatsapp, $mail, $fac, $lha, $ac_ta, $iso, $ur, $da, $mid)) or (print_r($con->errorInfo()));
