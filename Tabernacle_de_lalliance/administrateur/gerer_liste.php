...
$out_mem=$con->query("select * from membre and branche='$bid';") or die (print_r($con->errorInfo()));
$count_mem=$out_mem->rowCount();

$groupe_out=$con->query("select * from groupe
	inner join membre on groupe.leader=membre.mid where groupe.gid='$groupe' and branche='$bid';")or die (print_r($con->errorInfo()));
$groupe_out=$groupe_out->fetch();

$mem_nom=$con->query("select count(membre) as nombre from grou_mem
	where gid='$groupe' and membre.branche='$bid';")or die (print_r($con->errorInfo()));
