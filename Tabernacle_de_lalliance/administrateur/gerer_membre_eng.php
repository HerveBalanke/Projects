<?php
					...
					// include_once("session_admin.php");
		   			...
		   			$insert_membre=$con->prepare("insert into membre (nom, prenom, genre, dob, photo, nbre_denfant, pays, t_membre, profession, filiere, ecole, sit_mat, codetel, tel, codewhat, whatsapp, email, facebook, address, activite_ac, info_spi, per_urgence, date_adhesion, branche)
		   										values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);") or (print_r($con->errorInfo()));
		   			$insert_membre->execute(array( $nom, $prenom, $genre, $date, '-', $ne, $pays, $sta, $prof, $fil, $ecole, $sim, $codetel, $tel, $codewhat, $whatsapp, $mail, $fac, $lha, $ac_ta, $iso, $ur, $da, $bid)) or (print_r($con->errorInfo()));
