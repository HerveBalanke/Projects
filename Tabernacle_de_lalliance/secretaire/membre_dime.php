...
require_once("../setup/connection.php");
  ...
  $out_dime=$con->query("select * from membre inner join dime on membre.mid=dime.membre where mid='$mid' and branche='$bid' ;") or die (print_r($con->errorInfo()));
 $fetch_dime_info=$out_dime->fetch();


 $out_dime_fiche=$con->query("select distinct extract(year from date) as year , extract(month from date) as month FROM dime where membre='$mid' and branche='$bid' ORDER BY date desc;
        ...
        <div class="row">
          <div class="col-md-12">
					...
					<div class="panel panel-default">
                        ...
                        <a href='view_membre.php?mid=<?php echo $mid;?> 'id='retour' style="color:rgb(255, 213, 29);" title="Retournez les données du membre" class="pull-right"> <b> Retour </b> </a>
                      </div>
...
$arrData = array(
"chart"=> array(
        "caption"=> "Historique des Dimes - ".strtoupper($fetch_dime_info['mid'])." - ".strtoupper($fetch_dime_info['nom'])." ".strtoupper($fetch_dime_info['prenom']) ,
        //"subcaption"=> "",
        "xaxisname"=> "Date",
    ...
    <div class="panel-body">
                            ...
                            <tbody>
                              ...
                              $date_q = $fetch_dime_fiche['year']."-".$fetch_dime_fiche['month'];
                                  ?>
                              <tr>
                                <td class="text-center"><a href='bilan_fiche_dime_mem.php?date=<?php echo $date_q;?>& mid=<?php echo $mid; ?>' ><?php echo $date_q;?> </a></td>
                                <td class="text-center"><?php
                                $year=$fetch_dime_fiche['year'];
                                $month=$fetch_dime_fiche['month'];
                                $out_dime_fiche_sum=$con->query("select sum(montant) as sum FROM dime where membre='$mid' and year(date)='$year' and month(date)='$month' and branche='$bid'; ") or die (print_r($con->errorInfo()));
