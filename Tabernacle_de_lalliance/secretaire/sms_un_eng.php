...
// */
// $username = 'Herve_Balanke';
// $password = 'YOSEPODALH9';
// /*
...
try {
   ...
   if ($messageResponse instanceof MessageResponse) {
        $query=$con->query("insert into sms (sms, date, branche) values('$sms', '$date', '$bid')") or (print_r($con->errorInfo()));
        $out_sms=$con->query("select * from sms order by sid desc limit 1;") or (print_r($con->errorInfo()));
        $fetch_sms=$out_sms->fetch();
        $in_sms=$fetch_sms['sid'];
        $query=$con->query("insert into sms_envoi (smsid, tel) values('$in_sms', '$tel')") or (print_r($con->errorInfo()));
       // echo $tel;
        echo "OK";
