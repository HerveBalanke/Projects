...
$cid=$_GET["cid"]; 
$lead_det=$con->query("select * from cellule inner join zone_cel on cellule.zone_cel=zone_cel.zid_cel where cid='$cid';") or die(print_r($con->errorInfo()));
$fetch_lead=$lead_det->fetch();
?>
        ...
        <div class="col-md-3"></div>
          <div class="col-md-6">
										  	...
										  	<div class="panel panel-default">
												 <div class="panel-heading" style="color:rgb(255, 213, 29); background:rgb(130, 0, 31);"> <b>MODIFIER UN LEADER</b></div>


													<div class="panel-body">
         	<form role="form" id="form" method="POST" action="ajouter_porteur.php">
              <div id="flash"></div>
                <input type="hidden" class="form-control" id="cid" value="<?php  echo $fetch_lead["cid"]; ?>">
                <div class="form-group">
                  <label for="nom">Nom:</label>
                  <input type="text" class="form-control" id="nom" value="<?php  echo $fetch_lead["lnom"]; ?>" style="text-transform: capitalize;" placeholder="Entrez Nom" maxlength="30">
                  <div id='check_nom' style="color:red"></div>
                </div>
                <div class="form-group">
                  <label for="prenom">Prenom:</label>
                  <input type="text" class="form-control" id="prenom" value="<?php  echo $fetch_lead["lprenom"]; ?>" style="text-transform: capitalize;" placeholder="Entrez Prenom" maxlength="30">
                  <div id='check_prenom' style="color:red"></div>
                </div>
                 <div class="form-group">
                  <label for="tel">Telephone:</label>
                  <input type="telephone" class="form-control" id="tel" value="<?php  echo $fetch_lead["tel"]; ?>" placeholder="Entrez le N° de Telephone" maxlength="10">
                  <div id='check_tel' style="color:red"></div>
                </div>
                <div class="form-group">
                  <label for="email">Email:</label>
                  <input type="email" class="form-control" id="email"  value="<?php  echo $fetch_lead["email"]; ?>" placeholder="Entrez L'address e-mail" maxlength="30">
                 ...
                 <div class="form-group">
                  ...
                  <div> 
                     <select class="form-control" id="zone" >
                      <option value="<?php  echo $fetch_lead["zid_cel"]; ?>"><?php  echo $fetch_lead["zone"]; ?></option>
            ...
            <a href="ajouter_leader.php">Annuler</a>&nbsp;&nbsp;&nbsp;
    ...
    $("#nom").focus();
          ...
          $('#zone').on('change',function(){
              ...
              zone = zone_alt;
                    ...
                    var dataString ="cid=" + cid + "&nom="+ nom + "&prenom=" + prenom + "&tel=" + tel + "&email=" + email + "&zone="+ zone+ "&zone_alt="+ zone_alt ;
                    // alert(dataString);
                    $('#lod').show();
                    $.ajax({
                    type: "POST",
                    url: "modifier_leader_eng.php",
                    data:dataString,
                    cache:true,
                    processData:false,
                    success: function(out){
                    {
                      ...
                      $("#flash").html(out);
                          ...
                          $('#form').clearForm();
                          $('#lod').hide();
                          bootbox.alert("<strong>Compte modifié! Les données ont été envoyé à l'adresse téléphonique du Leader</strong>", function(result) {
                          window.location.href ='ajouter_leader.php';
                          });
                          } else if (out=="envoi_NO"){
                          $('#form').clearForm();
                          $('#lod').hide();
                          bootbox.alert("<strong>Compte modifié! Les données n'ont pas éte envoyé à l'addresse téléphonique du Leader Vérifier la connection internet!</strong>", function(result) {
                           window.location.href ='ajouter_leader.php';
                          });
                          ...
                          $('#lod').hide();
                          bootbox.alert("<strong>Ce numéro est en cours d'utilisation, veuillez entrer un autre</strong>");
                          $("#tel").focus();
