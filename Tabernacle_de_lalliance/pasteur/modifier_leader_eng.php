<?php
							...
							$cid=$_POST["cid"];
		   					...
		   					$check_tel=$con->query("select * from cellule where cid!='$cid'and tel='$tel';") or (print_r($con->errorInfo()));
		   					...
		   					$check_email=$con->query("select * from cellule where email='$email' and cid!='$cid';") or (print_r($con->errorInfo()));
							...
							do{
		   					...
		   					$in=$con->prepare("update cellule set lnom=?, lprenom=?, uname=?, pass=?, tel=?, email=?, zone_cel=?, branche=?, desactivate=? where cid=? ;") or (print_r($con->errorInfo()));
		   					$in->execute(array( $nom, $prenom, 'LeaderCellule', $pass, $tel, $email, $zone_cel, $bid, 'off', $cid)) or (print_r($con->errorInfo()));
