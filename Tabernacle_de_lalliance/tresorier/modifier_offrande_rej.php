...
$out_off=$con->query("select * from offrande 
	inner join evenement on offrande.evenement=evenement.eid where oid ='$oid' and tresorier='$treso' and (usher='non' OR admin='non') and branche='$bid';") or die(print_r($con->errorInfo()));
$fetch_off=$out_off->fetch();
$count_row=$out_off->rowCount();
if($count_row<=0){
	header('location:alert_used_entrees_mod.php');
exit();
}
?>
...
success: function(result){
	...
	$("#flash").fadeIn(400).html(bootbox.alert("<b>Modification terminée.</b>" , function(result) {
			window.location.href ="entrees_rejetees_ush.php";
		}));
