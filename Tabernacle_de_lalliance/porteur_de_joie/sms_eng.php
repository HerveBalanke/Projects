...
for ($i = 0; $i < $count_tel ; $i++) {
   ...
   // $mesg->setTime(date('Y-m-d H:i:s', strtotime('+1 week')));*/
    ...
    if ($messageResponse instanceof MessageResponse) {
        ...
        for ($i = 0; $i < $count_tel ; $i++) {
    $tel=$choix_array[$i];
    $query=$con->query("insert into sms_porteur (sms, date) values('$sms', '$date')") or (print_r($cons->errorInfo()));
    ...
    $query=$con->query("insert into sms_porteur_envoi (sms_pide, tel) values('$in_sms', '$tel')") or (print_r($cons->errorInfo()));
    }
        echo "OK";
