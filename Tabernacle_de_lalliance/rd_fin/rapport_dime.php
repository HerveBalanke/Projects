...
$bid=$_GET['bid'];
 $out_dime_fiche=$con->query("select distinct extract(year from date) as year , extract(month from date) as month FROM dime where 
  tresorier!='-' and usher='oui' and admin='oui' and branche='$bid' ORDER BY date desc;
...
$out_dime=$con->query("select * from (select distinct extract(year from date) as year , extract(month from date) as month FROM dime  where tresorier!='-' and usher='oui' and admin='oui' and branche='$bid' ORDER BY date desc limit 6) as tab order by tab.year, tab.month asc;") or die (print_r($con->errorInfo()));
...
$out_dime_c_sum=$con->query("select sum(montant) as sum FROM dime where year(date)='$yearc' and month(date)='$monthc' and tresorier!='-' and usher='oui' and admin='oui' and branche='$bid';") or die (print_r($con->errorInfo()));
    ...
    <div class="panel-body">
                            ...
                            <tbody>
                              ...
                              $date_q = $fetch_dime_fiche['year']."-".$fetch_dime_fiche['month'];
                                  ?>
                              <tr>
                                <td class="text-center"><a href='rapport_fiche_dime.php?date=<?php echo $date_q;?>&bid=<?php echo $bid ;?>' ><?php echo $date_q;?> </a></td>
                                <td class="text-center"><?php
                                $year=$fetch_dime_fiche['year'];
                                $month=$fetch_dime_fiche['month'];
                                $out_dime_fiche_sum=$con->query("select sum(montant) as sum FROM dime where year(date)='$year' and month(date)='$month' and tresorier!='-' and usher='oui' and admin='oui' and branche='$bid'; ") or die (print_r($con->errorInfo()));
