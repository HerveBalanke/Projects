  ...
  visibility:hidden;
    ...
    <?php
          ...
          try{
           ...
           $nid=$_GET['nid'];
           $query3=$cons->query("select * from nurse where nid='$nid';");
            $hfetch3=$query3->fetch();
$hfname=$hfetch3['fname'];
$hlname=$hfetch3['lname'];
            /*$out=$cons->query("select * from vaccination;") or die (print_r($out->errorInfo()));
            $row= $out-> rowCount();
             $list=$out->fetch();*/

             $query=$cons->query("select * from vaccination where nurse='$nid';");
             $row=$query->rowCount();
...
<div class="section">
      <div class="container">
        <div class="row">
          <div class="col-md-12">
            <h1 class="text-center"><?php echo $hfname." ".$hlname ; ?> Vaccinations</h1> <br/>
        <div id='replace' >

<input type="hidden" class="form-control" id="nid" value="<?php $nid; ?>">
...
var dataString= "search="+ search+ "&nid=" + nid;
//alert(dataString);
