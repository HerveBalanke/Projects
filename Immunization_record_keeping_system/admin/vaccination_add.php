...
<?php
						...
						$ustock=$cons->query("select qty from quantity where vcode='$vcode';");
						$ufetch=$ustock->fetch();
						$nstock=$ufetch['qty']-1;
						$instock=$cons->query("update quantity set qty='$nstock' where vcode='$vcode'; ");
						...
						if($nodss==0 || $nodss<0){
							$update=$cons->query("update appointment set status='fulfilled' where vaccination='$vacid';")or die(print_r($vac->errorInfo()));
...
$mesg = new Message();
  ...
  <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
  <strong>SMS not sent, Please check your Connection</strong>
</div>";
