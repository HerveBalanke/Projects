...
<br/>
               <div id='printtable'>
            <h2 class="text-center">Inventory of Vaccine</h2>
...
<tr>
        ...
        if( $fetch["qty"]<=50){
           ...
           $query=$cons->query("select * from quantity inner join vaccine on quantity.vcode=vaccine.vcode;");
          $res=$query->fetch();
          $email=$res["manuemail"];

          $check=$cons->query("select * from message where vcode='$vac' and manuemail='$email' and dates='$today';");
          $rcount=$check->rowCount();
          if($rcount<0 || $rcount==0){

          
         

          $message="Hello, we need a supply of 300 doses of $vac";
...
$mail->Subject = 'Vaccine Supply';
$mail->Body    = $message;


if($mail->send()) {

$insert=$cons->query("insert into message (vcode,  dates, manuemail) values ('$vac', '$today', '$email');") or die(print_r($cons->errorInfo()));
        ...
        print "<span style='color:red;'>".$fetch["qty"]."</span>"; 
      }else{
...
<?php
 ...
 <a href='chart.php' class="btn">Chart <span class="glyphicon glyphicon-resize-full"></span> </a>&nbsp;&nbsp;&nbsp;
 <a href='inventory_hist.php' class="btn">Stock Update History <span class="glyphicon glyphicon-list-alt"></span> </a>
          </div>
          <div class="col-md-2">
          </div>
          </div>
